...
public abstract class CircleStyleGraph extends Graph {
    ...
    public static final double ANGLE_TO_RADIAN_K = 0.0174533;
    ...
    protected int getNumOfVerticies() {
        return this.graph.length;
    }

    protected double getOneVertexArcAngle() {
        return CIRCLE_ANGLE / this.graph.length;
    }
    ...
    private int vertexRadiusBinarySearch(double l, double r, double placeRadius) {
        ...
        while (r - l > 0.001) {
            ...
            if (2 * Math.PI * vertexRadius * getNumOfVerticies() > placeRadius * 0.90) {
                r = mid;
            } else {
                l = mid;
            }
